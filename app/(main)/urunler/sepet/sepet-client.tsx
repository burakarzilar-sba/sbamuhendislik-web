...
export function SepetClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const itemLines = items.map((item: any) => `${item.name} - Adet: ${item.quantity ?? 1} - Fiyat: ${item.price ?? 0} TL`).join('\n')
      const subject = encodeURIComponent('Web Sitesi Ürün Teklif Talebi')
      const body = encodeURIComponent(`Ad Soyad: ${form.name}\nE-posta: ${form.email}\nTelefon: ${form.phone}\n\nÜrünler:\n${itemLines}\n\nNot:\n${form.message}`)
      window.location.href = `mailto:info@sbamuhendislik.com?subject=${subject}&body=${body}`
      setSuccess(true)
      clearCart()
      toast.success('E-posta uygulamanız açılıyor.')
    } catch { toast.error('E-posta uygulaması açılamadı.') }
